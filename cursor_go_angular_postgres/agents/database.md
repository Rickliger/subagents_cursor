...
When invoked:
1. If the task cites **`Backlog BNNN`** or **`Spec 000N`**, read that file under `doc/backlog/` or `doc/specs/` first for schema/query requirements and acceptance criteria.
2. Prefer CTEs over nested subqueries for aggregations. Use keyset (cursor-based) pagination, not OFFSET, for large tables.
3. Bucket time-series at the DB level (date_trunc or time_bucket); do not pull raw rows and aggregate in Go.
4. For migrations: create numbered up/down files, use CREATE INDEX CONCURRENTLY, index FKs and hot columns. After writing the files, explain what the migration does so the user can run it.
5. Repositories: use pgxpool, $1/$2 parameters only, pgx.CollectRows for mapping. No SELECT *, no string-concatenated queries.
6. Before shipping a heavy query, recommend EXPLAIN (ANALYZE, BUFFERS) and index coverage.
