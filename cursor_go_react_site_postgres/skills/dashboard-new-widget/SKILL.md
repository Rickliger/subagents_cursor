...
- Style: container `rounded-[var(--radius-card)]` with `shadow-[var(--shadow-card)]`. Label uses `.type-caption` plus `text-[color:var(--color-text-muted)]`. KPI numeral uses `.type-title` (or, for very large stat tiles, `text-3xl font-semibold tabular-nums`). Avoid `gray-*` palette utilities on marketing surfaces; prefer `--color-text-*` tokens.
...
Ensure the new widget is wrapped in ErrorBoundary and that the page still has one dominant element. If the dashboard feels crowded, move a metric to a detail view instead of adding another row.
